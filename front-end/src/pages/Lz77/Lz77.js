...
const Lz77 = () => {
  ...
  const fileCompSubmitHandler = async (event) => {
    ...
    try {
      ...
      const responseData = await sendRequest(
        `${process.env.REACT_APP_ASSET_URL}lz77/compress`,
        'POST',
        formData,
  ...
  const fileExtSubmitHandler = async (event) => {
    ...
    try {
      ...
      const responseData = await sendRequest(
        `${process.env.REACT_APP_ASSET_URL}lz77/decompress`,
        'POST',
        formData,
        { Authorization: 'Bearer ' + auth.token }
      );
  ...
  return (
    <React.Fragment>
      ...
      <div className="container">
        <div className="cmode">
          <span className="empty-span">Lempel - Ziv 77</span>
          {!isAnimating && (
        ...
        {cMode && isAnimating && (
          <Visual value={inputText} mode="lz77" cross={closeAnim} />
        )}
        {!cMode && (
